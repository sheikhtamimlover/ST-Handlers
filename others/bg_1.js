...
module.exports = {
  ...
  ST: async function ({ message, args, event }) {
    ...
    function loadConfig() {
      try {
        ...
        return { enabled: true, logThread: "886960563901648", logJoin: true, logLeave: true, logKick: true, logCommandUse: true, excludeCommands: ["activit", "aclog"] };
      } catch {
        return { enabled: true, logThread: "886960563901648", logJoin: true, logLeave: true, logKick: true, logCommandUse: true, excludeCommands: ["activit", "aclog"] };
      }
    }
  ...
  onChat: async function ({ event, api, threadsData, usersData, commandName }) {
    ...
    if (config.excludeCommands && config.excludeCommands.includes(commandName.toLowerCase())) {
      return;
    }
