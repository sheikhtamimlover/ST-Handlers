const fs = require("fs-extra");
const path = require("path");

let cachedData = null;
const dataPath = path.join(__dirname, '..', '..', 'cache', 'autoreact_data.json');

async function loadCache() {
  if (cachedData !== null) return cachedData;
  
  try {
    const fileContent = await fs.readFile(dataPath, 'utf8');
    cachedData = fileContent ? JSON.parse(fileContent) : { enabled: false };
  } catch {
    cachedData = { enabled: false };
  }
  return cachedData;
}

async function saveCache(data) {
  cachedData = data;
  await fs.writeFile(dataPath, JSON.stringify(data, null, 2));
}

module.exports = {
  config: {
    name: "trimoji",
    aliases: ["tmj", "tri-react"],
    version: "1.0",
    author: "ST | Sheikh Tamim",
    countDown: 5,
    role: 2,
    description: "Enable/disable auto-trimoji reaction (Admin only)",
    category: "admin",
    guide: {
      en: "{pn} on - Enable trimoji auto react\n{pn} off - Disable trimoji auto react\n{pn} - Check status"
    }
  },

  onStart: async function ({ message, args, event, api }) {
    try {
      await fs.ensureFile(dataPath);
      const fileContent = await fs.readFile(dataPath, 'utf8');
      if (!fileContent) {
        await fs.writeFile(dataPath, JSON.stringify({ enabled: false }, null, 2));
      }
      await loadCache();
    } catch (error) {
      await fs.ensureDir(path.dirname(dataPath));
      await fs.writeFile(dataPath, JSON.stringify({ enabled: false }, null, 2));
      cachedData = { enabled: false };
    }

    const data = await loadCache();

    if (!args[0]) {
      const status = data.enabled ? "enabled ✅" : "disabled ❌";
      return message.reply(`🔔 Trimoji Auto Reaction is currently **${status}**.`);
    }

    const choice = args[0].toLowerCase();

    if (choice === "on") {
      data.enabled = true;
      await saveCache(data);
      api.setMessageReaction("✅", event.messageID, () => {}, true);
      return message.reply("✅ Trimoji auto-react has been **enabled globally**!");

    } else if (choice === "off") {
      data.enabled = false;
      await saveCache(data);
      api.setMessageReaction("❌", event.messageID, () => {}, true);
      return message.reply("❌ Trimoji auto-react has been **disabled globally**!");

    } else {
      return message.reply("❌ Invalid command! Use:\n• trimoji on\n• trimoji off");
    }
  },

  onChat: async function ({ event, api }) {
    if (event.senderID === api.getCurrentUserID()) return;

    const data = await loadCache();
    if (!data.enabled) return;

    const reactions = [
      "😊‍😍‍😆‍😮‍😆‍🔪‍❤️‍🔥‍❤️‍🙊‍😿‍😅",
      "❤‍💖‍💗‍💓‍😆‍🔪‍🙊‍💖‍🙉‍😻‍😿",
      "🔥‍⭐‍✨‍🌟‍💫‍🔪‍💞‍💖‍😿‍🙊‍🙉",
      "👍‍👎‍😠‍😮‍😆‍🔪‍🙉‍😿‍💓‍💝‍💋",
      "💙‍💚‍💛‍💜‍🖤‍🔪‍🙊‍😿‍😻‍😿‍🙊",
      "💕‍💞‍💓‍💗‍💖‍🔪‍😻‍🙉‍💝‍😹‍😿",
      "😮‍😆‍😍‍😊‍😢‍🔪‍😽‍🤬‍☠️‍😠‍🥱",
      "💯‍🔥‍✨‍⭐‍🌟‍🔪‍😧‍😱‍😨‍😭‍😧",
      "😆‍😮‍😢‍😠‍😍‍🔪‍😣‍😱‍😧‍🥺‍😨",
      "💫‍🌟‍✨‍⭐‍🔥‍🔪‍😞‍😱‍😧‍🥺‍😨",
      "😎‍🤩‍🥳‍🤔‍🙄‍🔪‍🙄‍🫥‍🙄‍😲‍😒",
      "🤯‍😱‍😳‍🥵‍🥶‍🔪‍🤫‍🤐‍🤨‍😌‍🫥",
      "😴‍🤤‍😪‍😵‍🤕‍🔪‍🥶‍🙄‍🙄‍🤤‍🥶",
      "🤗‍🤝‍🙌‍👏‍👐‍🔪‍😠‍🥱‍😣‍😧‍🥺",
      "💩‍👻‍💀‍☠‍🔪‍😐‍💓‍💝‍🙉‍💝‍🙉",
      "🤖‍🎃‍🎄‍🎁‍🎉‍🔪‍💞‍💔‍❤️‍💝‍💕",
      "🍎‍🍊‍🍌‍🍉‍🍇‍🔪‍👹‍👺‍😿‍🤬‍😺",
      "🥑‍🥦‍🥕‍🌽‍🌶️‍🔪‍😹‍💝‍😹‍😿‍👺",
      "⚽‍🏀‍🏈‍⚾‍🎾‍🔪‍😻‍😺‍👹‍😿‍🤬",
      "🚗‍🚕‍🚙‍🚌‍🏎️‍🔪‍🥵‍🫤‍😮‍🤢‍😴",
      "✈‍🚀‍🛸‍🛶‍⛵‍🔪‍🤐‍🤔‍🥶‍🥺‍🥵",
      "🎵‍🎶‍🎷‍🎸‍🥁‍🔪‍🤐‍🤨‍😐‍🤫‍😙",
      "📚‍📖‍📝‍🖊️‍📌‍🔪‍🐸‍😅‍😄‍🙃‍🫠",
      "💡‍🔧‍🔨‍⚙️‍🧰‍🐸‍😅‍😄‍🙃‍🫠‍🔪"
    ];

    try {
      const random = reactions[Math.floor(Math.random() * reactions.length)];
      api.setMessageReaction(random, event.messageID, () => {}, true);
    } catch (err) {
      console.log("Trimoji auto react error:", err);
    }
  }
};