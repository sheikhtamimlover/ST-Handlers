      ...
      for (let frame = 0; frame < frames; frame++) {
        ...
        for (let i = 0; i < 3; i++) {
          ...
          if (frame < reelStopFrames[i]) {
            ...
            if (reelPositions[i] >= symbols.length * symbolHeight * 10) {
              reelPositions[i] = 0;
            }
          } else {
            const targetIdx = symbols.indexOf(finalSymbols[i]);
            const targetPosition = targetIdx * symbolHeight;
            const currentMod = reelPositions[i] % (symbols.length * symbolHeight);
            
            let diff = targetPosition - currentMod;
            if (diff < 0) diff += symbols.length * symbolHeight;
            if (diff > symbols.length * symbolHeight / 2) diff -= symbols.length * symbolHeight;
          ...
          const visibleSymbols = 8;
          for (let j = -2; j < visibleSymbols; j++) {
            const normalizedPos = reelPositions[i] % (symbols.length * symbolHeight);
            const symbolY = reelY + reelHeight / 2 + (j * symbolHeight - normalizedPos);
            
            let symbolIdx = Math.floor((reelPositions[i] / symbolHeight + j)) % symbols.length;
