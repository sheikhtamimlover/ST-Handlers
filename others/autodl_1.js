  ...
  onChat: async function ({ message, event, usersData }) {
    ...
    try {
      ...
      if (finalUrl.includes("youtube.com") || finalUrl.includes("youtu.be")) {
        ...
        try {
          usedUrl = proxyUrl;
          const streamResp = await axios({ url: proxyUrl, method: "GET", responseType: "stream", timeout: 30000 });
          const writer = fs.createWriteStream(filePath);
          streamResp.data.pipe(writer);
          await new Promise((resolve, reject) => { writer.on("finish", resolve); writer.on("error", reject); });
        } catch {
          usedUrl = downloadUrl;
          const streamResp = await axios({ url: downloadUrl, method: "GET", responseType: "stream", timeout: 30000 });
          const writer = fs.createWriteStream(filePath);
          streamResp.data.pipe(writer);
          await new Promise((resolve, reject) => { writer.on("finish", resolve); writer.on("error", reject); });
        }
