    ...
    try {
      ...
      for (let i = 0; i < frames; i++) {
        ...
        for (let p = 0; p < 50; p++) {
          ...
          const pSize = Math.sin(t * Math.PI * 3 + p) * 2.5 + 2;
        ...
        for (let ring = 0; ring < 4; ring++) {
          const ringRadius = avatarSize / 2 + 8 + ring * 6;
        ...
        for (let s = 0; s < sparkCount; s++) {
          ...
          const sparkSize = Math.sin(t * Math.PI * 10 + s) * 5 + 7;
          const sparkAlpha = Math.sin(t * Math.PI * 7 + s * 0.9) * 0.5 + 0.5;
          
          const sparkGrad = ctx.createRadialGradient(sparkX, sparkY, 0, sparkX, sparkY, sparkSize * 2.5);
