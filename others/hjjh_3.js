  ...
  ST: async function({ message, event, api, getLang, usersData }) {
    try {
      ...
      for (let i = 0; i < frames; i++) {
        ...
        const colorIndex = Math.floor(t * colors.length * 2) % colors.length;
        const nextColorIndex = (colorIndex + 1) % colors.length;
        const colorProgress = (t * colors.length * 2) % 1;
        ...
        for (let p = 0; p < 35; p++) {
          const px = (width * ((p * 0.618 + t * 0.2) % 1));
          const py = (height * ((p * 0.382 + t * 0.15) % 1));
        ...
        const shineIntensity = Math.sin(t * Math.PI) * 0.6 + 0.2;
        
        const shineGrad = ctx.createLinearGradient(shineX - shineWidth, cardY, shineX + shineWidth, cardY + cardH);
        shineGrad.addColorStop(0, "rgba(255, 255, 255, 0)");
        shineGrad.addColorStop(0.3, `rgba(255, 255, 255, ${shineIntensity * 0.3})`);
        shineGrad.addColorStop(0.5, `rgba(255, 255, 255, ${shineIntensity})`);
        shineGrad.addColorStop(0.7, `rgba(255, 255, 255, ${shineIntensity * 0.3})`);
        ...
        const nameGrad = ctx.createLinearGradient(cardX + 240, cardY + 115, cardX + 600, cardY + 150);
        ...
        for (let g = 0; g < glowCount; g++) {
          ...
          const glowSize = Math.max(1, Math.sin(t * Math.PI * 6 + g) * 4 + 6);
          const glowAlpha = Math.sin(t * Math.PI * 4 + g * 0.7) * 0.35 + 0.45;
        ...
        const corners = [
          [cardX + 25, cardY + 25],
          [cardX + cardW - 45, cardY + 25],
          [cardX + 25, cardY + cardH - 45],
          [cardX + cardW - 45, cardY + cardH - 45]
        ];
