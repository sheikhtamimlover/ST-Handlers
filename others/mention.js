module.exports = {
  ...
  onStart: async function({ api, event, args, message, usersData }) {
    ...
    if (isAll) {
      ...
      // Reply-based single mention
      if (!messageReply) {
        return message.reply("❌ Please reply to a user to mention them!");
      }

      const replyID = messageReply.senderID;
      const nickname = userNicknames[replyID] || await usersData.getName(replyID);
      ...
      return message.reply({
        body,
        mentions: [{ tag: `@${nickname}`, id: replyID }]
      });
    }
  }
};