...
module.exports = {
  config: {
    ...
    countDown: 0,
    role: 2,
    description: "Anti-unsend system to log deleted messages",
    category: "system",
    guide: "{pn} on/off - Enable/disable\n{pn} setthread <tid> - Set log thread\n{pn} info - Show status\n{pn} clear - Clear cache"
  },

  onLoad: function () {
    const configFile = path.join(__dirname, "..", "..", "antiunsend_config.json");
  ...
  ST: async function ({ message, args, event }) {
    ...
    if (args[0] === "setthread" || args[0] === "thread") {
      if (!args[1]) {
        return message.reply("❌ Usage: antiun setthread <threadID>");
      }
    ...
    if (args[0] === "info" || args[0] === "status") {
      ...
      return message.reply(
        ...
        `💡 Commands:\n` +
        `• antiun on/off\n` +
        `• antiun setthread <threadID>\n` +
        `• antiun clear - Clear cache`
      );
    }
    ...
    if (event.type === "message_unsend") {
      ...
      try {
        ...
        try {
          ...
          senderName = senderInfo?.name || await usersData.getName(messageData.senderID) || "Unknown User";
        } catch {
          try {
            senderName = await usersData.getName(messageData.senderID) || "Unknown User";
          } catch {
